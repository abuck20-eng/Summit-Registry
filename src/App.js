...
function GradeSlider({ grades, value, onChange }) {
  ...
  const getIdx = (clientX) => {
    ...
    return Math.round(Math.max(0, Math.min(1, (clientX - rect.left) / rect.width)) * (grades.length - 1));
  };
  ...
  return (
    <div style={{ padding:"0 24px", userSelect:"none" }}>
      ...
      <div ref={trackRef} onPointerDown={onDown} onPointerMove={onMove} onPointerUp={onUp}
        ...
        {grades.map((_,i) => (
          <div key={i} style={{ position:"absolute", left:`${(i/(grades.length-1))*100}%`, transform:"translateX(-50%)", width:i===di?2:1, height:i===di?12:5, background:i===di?"#f0ede8":"#252525", bottom:10, borderRadius:1 }} />
        ))}
...
function Chip({ label, selected, onToggle, accent }) {
  return (
    <button onClick={onToggle} style={{
      padding:"7px 14px", borderRadius:20, fontSize:12, fontWeight:600, letterSpacing:"0.04em",
      cursor:"pointer", fontFamily:"'DM Mono',monospace", border:"none", transition:"all 0.12s", flexShrink:0,
      background: selected ? (accent || "#f0ede8") : "#1e1e1e",
      color: selected ? (accent ? "#fff" : "#0e0e0e") : "#484848",
    }}>{label}</button>
  );
}
...
function SentButton({ disabled, onSent, onFirstGo }) {
  ...
  const handlePress = () => {
    ...
    if (gap < DOUBLE_TAP_MS && gap > 0) {
      ...
      setArmed(true);
      tapTimer.current = setTimeout(() => { setArmed(false); onSent(); }, DOUBLE_TAP_MS);
    }
  };
  ...
  return (
    <button onClick={handlePress} style={{
      ...S.outcomeBtn, background: armed ? "#7ecf82" : "#4caf50", color:"#fff",
      opacity: disabled ? 0.2 : 1, transform: armed ? "scale(0.97)" : "scale(1)",
      position:"relative", overflow:"hidden", transition:"background 0.1s, opacity 0.12s, transform 0.08s",
    }}>
      {armed ? "⚡ ?" : "SENT"}
...
function DetailSheet({ entry, onSave, onDismiss, saving }) {
    ...
    <div style={S.overlay} onClick={onDismiss}>
      <div style={S.sheet} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:20 }}>
          <span style={{ color: isSent ? "#4caf50" : "#777", fontSize:16 }}>{isSent ? "✓" : "◎"}</span>
          <div style={S.sheetTitle}>{displayName(entry)}</div>
          <div style={S.sheetGrade}>{entry.grade}</div>
        </div>
        {isSent && (
          <div style={S.tagSection}>
            <div style={S.chipRow}>
              <Chip label={firstGo ? "⚡ First go" : "⚡ First go?"} selected={firstGo} onToggle={() => setFirstGo(p=>!p)} accent="#a06010" />
            </div>
        ...
        <div style={S.sheetBtns}>
          <button style={S.btnSecondary} onClick={onDismiss}>cancel</button>
          <button style={{ ...S.btnPrimary, opacity: saving ? 0.6 : 1 }} onClick={() => onSave({ note, angles, styles, first_go: firstGo })} disabled={saving}>
            {saving ? "saving..." : "save"}
          </button>
        </div>
      </div>
...
function LiveLogRow({ log, isNoteWindow, onTap }) {
  ...
  useEffect(() => {
    ...
    const tick = (now) => {
      const pct = Math.max(0, 100 - ((now - startRef.current) / NOTE_WINDOW_MS) * 100);
      setProgress(pct);
      if (pct > 0) rafRef.current = requestAnimationFrame(tick); else setProgress(0);
    };
    ...
    <div style={S.logRowWrap}>
      <div style={S.logRow} onClick={onTap}>
        <div style={{ display:"flex", alignItems:"flex-start", gap:10, flex:1, minWidth:0 }}>
          <span style={{ ...S.dot, background: log.outcome==="sent" ? "#4caf50" : "#444", marginTop:5, flexShrink:0 }} />
          <div style={{ minWidth:0 }}>
            ...
            {log.note
              ...
              : hasTags
                ...
                : showWindow
                  ? <div style={S.addPromptActive}>+ add details · tap</div>
      ...
      {showWindow && (
        <div style={{ height:2, background:"#181818", borderRadius:1 }}>
          <div style={{ width:`${progress}%`, height:"100%", background:"#2e2e2e", borderRadius:1, transition:"width 0.08s linear" }} />
        </div>
      )}
    </div>
  );
  ...
  return (
    <div style={S.detailRow} onClick={onTap}>
      <div style={S.detailTop}>
        <span style={{ ...S.outcomeIcon, color: log.outcome==="sent" ? "#4caf50" : "#555" }}>{log.outcome==="sent"?"✓":"◎"}</span>
        <span style={S.detailName}>
          {displayName(log)}
          {log.first_go && <span style={{ fontSize:10, color:"#a06010", marginLeft:6 }}>⚡</span>}
        </span>
...
export default function App({ user, onSignOut }) {
  ...
  const commitLog = async (outcome, firstGo = false) => {
    if (!selectedGrade || !activeSession) return;
    ...
    if (saved) {
      setLogs(prev => prev.map(l => l.id === tempEntry.id ? { ...saved, is_gym: environment === "gym" } : l));
  ...
  const saveDetail = async ({ note, angles, styles, first_go }) => {
    setSheetSaving(true);
    const updates = { note, angles, styles, first_go };
  ...
  const endSession = async () => {
    ...
    setViewingSession(done);
    setScreen("summary");
  };

  const sends = logs.filter(l => l.outcome === "sent");
  const flashes = logs.filter(l => l.first_go === true);
  const Sheet = () => sheetEntry ? <DetailSheet entry={sheetEntry} onSave={saveDetail} onDismiss={() => setSheetEntry(null)} saving={sheetSaving} /> : null;
  ...
  if (screen === "home") return (
    <div style={S.app}>
      ...
      <div style={S.homeContainer}>
        ...
        {loadingSessions ? (
          <div style={S.emptyState}><div style={{ color:"#333", letterSpacing:"0.1em", fontSize:12 }}>loading...</div></div>
  ...
  if (screen === "setup") return (
    <div style={S.app}>
      <div style={S.pageContainer}>
        ...
        {setupStep===2 && <><div style={S.setupQ}>{environment==="gym"?"which gym?":"where at?"}</div><input style={S.locationInput} placeholder={environment==="gym"?"e.g. Movement RiNo":"e.g. Red Rock Canyon"} value={location} onChange={e=>setLocation(e.target.value)} autoFocus /><button style={{...S.startBtn,marginTop:32,opacity:location.trim()?1:0.35}} onClick={location.trim()?startSession:undefined}>LET'S CLIMB</button></>}
      </div>
    </div>
  );
    ...
    <div style={S.app}>
      ...
      <div style={S.sessionHeader}>
        <div>
          ...
          <div style={S.sessionMeta}>{logs.length} logged · {sends.length} sends{flashes.length>0?` · ${flashes.length} ⚡`:""}</div>
        </div>
        <button style={S.endBtn} onClick={endSession}>END</button>
      </div>
      <div style={S.nameSection}>
        <input style={S.nameInput} placeholder="climb name (optional)" value={climbName} onChange={e=>setClimbName(e.target.value)} />
      </div>
      ...
      <div style={S.outcomeSection}>
        <SentButton disabled={!selectedGrade} onSent={handleSent} onFirstGo={handleFirstGo} />
        <button style={{...S.outcomeBtn,...S.outcomeProject,opacity:selectedGrade?1:0.2}} onClick={() => commitLog("project")}>PROJECT</button>
      </div>
      {logs.length > 0 && (
...
const S = {
  ...
  emptyText:{ fontSize:13, letterSpacing:"0.12em" },
  ...
  sessionCardLocation:{ fontSize:16, fontWeight:600 },
  sessionCardMeta:{ fontSize:12, color:"#555", marginTop:3 },
  sessionCardDate:{ fontSize:12, color:"#404040" },
  ...
  statLabel:{ fontSize:10, color:"#484848", letterSpacing:"0.08em", marginTop:3, textTransform:"uppercase" },
  ...
  outcomeIcon:{ fontSize:15, width:18, textAlign:"center", flexShrink:0 },
  detailName:{ flex:1, color:"#bbb", fontSize:14 },
  detailGrade:{ fontSize:12, color:"#484848", fontWeight:600 },
  ...
  sessionLocation:{ fontSize:17, fontWeight:700, letterSpacing:"0.04em" },
  sessionMeta:{ fontSize:12, color:"#444", marginTop:3 },
  ...
  outcomeSent:{ background:"#4caf50", color:"#fff" },
  outcomeProject:{ background:"#161616", color:"#f0ede8", border:"1px solid #252525" },
  ...
  logName:{ fontSize:13, color:"#bbb" },
  logMeta:{ fontSize:11, color:"#484848", marginTop:2 },
  ...
  sheetGrade:{ fontSize:13, color:"#555", fontWeight:600 },
  tagSection:{ marginTop:20 },
  tagLabel:{ fontSize:10, color:"#383838", letterSpacing:"0.2em", textTransform:"uppercase", marginBottom:10 },
